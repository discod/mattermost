...
func TestGetServerLimits(t *testing.T) {
	...
	t.Run("unlicensed server shows hard-coded limits", func(t *testing.T) {
		...
		require.Equal(t, int64(2000), serverLimits.MaxUsersLimit)
		require.Equal(t, int64(2500), serverLimits.MaxUsersHardLimit)
	})
...
func TestIsAtUserLimit(t *testing.T) {
	...
	t.Run("unlicensed server", func(t *testing.T) {
		t.Run("below hard limit", func(t *testing.T) {
			...
			mockUserStore.On("Count", mock.Anything).Return(int64(2000), nil) // Under hard limit of 2500
		...
		t.Run("at hard limit", func(t *testing.T) {
			...
			mockUserStore := storemocks.UserStore{}
			mockUserStore.On("Count", mock.Anything).Return(int64(2500), nil) // At hard limit of 2500
			mockStore := th.App.Srv().Store().(*storemocks.Store)
			mockStore.On("User").Return(&mockUserStore)
		...
		t.Run("above hard limit", func(t *testing.T) {
			...
			mockUserStore := storemocks.UserStore{}
			mockUserStore.On("Count", mock.Anything).Return(int64(3000), nil) // Over hard limit of 2500
			mockStore := th.App.Srv().Store().(*storemocks.Store)
			mockStore.On("User").Return(&mockUserStore)
